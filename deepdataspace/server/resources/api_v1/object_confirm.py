...
class ObjectConfirmView(BaseAPIView):
    ...
    def post(self, request):
        ...
        if label is None:
            raise_exception(404, f"label_id[{label_id}] not found")

        image = Image(dataset_id).find_one({"id": image_id})
        if image is None:
            raise_exception(404, f"image_id[{image_id}] not found")
        ...
        for idx, obj in enumerate(objects):
            ...
            try:
                ...
                logger.warning(err)
                raise_exception(404, f"objects[{idx}] data structure mismatch")
